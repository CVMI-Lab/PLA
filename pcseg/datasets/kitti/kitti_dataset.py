...
class KittiDataset(OutdoorDataset):
    def __init__(self, dataset_cfg, class_names, training, root_path, logger=None, split=None):
        ...
        self.filename_list = ['_'.join(d.split('/')[-4:])[:-4] for d in self.data_list]
        ...
        self.logger.info(
            "Totally {} samples in {} set.".format(len(self.data_list), self.mode))
    ...
    def __getitem__(self, item):
        ...
        xyz, intensity, label, inst_label, binary_label, *others = self.load_data(index)

        pc_count = xyz.shape[0]
        origin_idx = np.arange(xyz.shape[0]).astype(np.int64)

        # === caption ===
        scene_name = self.filename_list[index]

        # get captioning data
        if self.training or self.dataset_cfg.get('FILTER_WITH_N_CAPTIONS', -1) != -1:
            image_corr_dict, image_name_dict = self.get_caption_image_corr_and_name_from_info(index, scene_name)
        ...
        data_dict = {
            ...
            'labels': label, 'inst_label': inst_label,
            'binary_labels': binary_label,
            'origin_idx': origin_idx, 'pc_count': pc_count,
            'caption_data': caption_data,
            'ids': index, 'scene_name': scene_name
        }
    ...
    def get_caption_image_corr_and_name_from_info(self, index, scene_name):
        ...
        if self.need_view_caption:
            ...
            cur_caption_idx = copy.deepcopy(self.point_caption_idx[scene_name])
            # assert scene_name == cur_caption_idx['scene_name']
            if len(cur_caption_idx) > 0:
                image_name_view, image_corr_view = zip(*cur_caption_idx.items())
            else:
        ...
        if self.need_entity_caption:
            cur_entity_point_caption_idx = copy.deepcopy(self.entity_point_caption_idx[scene_name])
            # assert scene_name == cur_entity_point_caption_idx['scene_name']
            if len(cur_entity_point_caption_idx) > 0:
                image_name_entity, image_corr_entity = zip(*cur_entity_point_caption_idx.items())
            else:
